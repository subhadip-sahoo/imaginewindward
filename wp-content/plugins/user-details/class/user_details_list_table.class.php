<?php
class user_details_list_table extends IWW_List_Table {
    ...
    function get_columns(){
        $columns = array(
            ...
            'user_nicename' => __('Nicename', 'user_details'),
            'status' => __('Status', 'user_details')
        );
        return $columns;
    }
    ...
    function prepare_items(){
        ...
        if (isset($_REQUEST['s']) && ! empty( $_REQUEST['s'] ) ){
                $args['s'] = $_REQUEST['s'];
                $status = ($args['s'] == 'Active')?0:1;
                $where = " WHERE user_email LIKE '%%".$args['s']."%%' OR user_nicename LIKE '%%".$args['s']."%%' OR status LIKE '%%".$status."%%'";
        }
        ...
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT ID, user_email, user_nicename, CASE user_status WHEN 0 THEN 'Active' WHEN 1 THEN 'Inactive' END as status FROM $table_name_users $where ORDER BY ".$args['orderby']." ".$args['order']." LIMIT %d OFFSET %d", $per_page, $args['offset']), ARRAY_A);
