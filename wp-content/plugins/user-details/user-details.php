        ...
        if ($item_valid === true) {
            if ($item['ID'] == 0) {
                ...
                if ( is_wp_error($stat) ) {
                    ...
                    update_user_meta($item['ID'],'account_status', 1);
                    $wpdb->update( "{$wpdb->prefix}users", array( 'user_status' => 0 ), array( 'ID' => $item['ID'] ) );
                    ...
                    $msg = "Dear User,<br/>Your account has been activated. Your login details are as follows<br/>Username: ".$username."<br/>Password: ".esc_sql($item['user_pass'])."<br/> Please login into your account to verify.";
...
<table cellspacing="2" cellpadding="5" style="width: 95%;" class="form-table">
    <tbody>
        ...
        <tr class="form-field">
            <th valign="top" scope="row">
                <label for="cuser_pass"><?php _e('Confirm Password', 'user_details')?></label>
            </th>
            <td>
                <input id="cuser_pass" name="cuser_pass" type="password" style="width: 95%" value="" class="code" />
            </td>
        </tr>
    </tbody>
</table>
...
function user_details_validate($item){
    ...
    if ($item['user_pass'] != $item['cuser_pass']) $messages[] = __('Password does not match', 'user_details');
