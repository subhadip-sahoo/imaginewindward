...
class user_details_list_table extends IWW_List_Table {
    ...
    function prepare_items(){
        ...
        $this->items = $wpdb->get_results($wpdb->prepare("SELECT ID, user_email, user_nicename, CASE user_status WHEN 1 THEN 'Active' WHEN 0 THEN 'Inactive' END as status FROM $table_name_users $where ORDER BY ".$args['orderby']." ".$args['order']." LIMIT %d OFFSET %d", $per_page, $args['offset']), ARRAY_A);
